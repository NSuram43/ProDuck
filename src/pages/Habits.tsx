...
import { exampleHabits, type Habit, type HabitInputType } from '../data/habits'
...
export default function Habits() {
  ...
  const categories = useMemo(() => {
    const set = new Set(exampleHabits.map((h) => h.category))
  ...
  function handleSubmitDay() {
    ...
    setMessage(pickMotivation(totalScore))
    ...
    <section className="space-y-6">
      ...
      {categories.map((cat) => (
        <div key={cat} className="bg-white rounded-lg border shadow-sm">
          <div className="px-4 py-2 border-b font-medium">{cat}</div>
...
function HabitInput({ type, value, onValue, max }: { type: HabitInputType; value: HabitValue | undefined; onValue: (v: HabitValue) => void; max?: number }) {
  if (type === 'checkbox') {
    return (
